...
Multiple variants are usually made possible by adding one or more [`ProductOptionGroups`](/reference/typescript-api/entities/product-option-group) to
...
A product does not have to use option groups. It may hold any number of variants with no options at all. Each
one is identified by its own name and SKU.
